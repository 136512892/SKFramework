    ...
    public class ScoreMaster : MonoBehaviour
    {
        ...
        public static ScoreMaster Instance
        {
            get
            {
                ...
                if (instance == null)
                {
                    ...
                    {
                        Log.Error(message: "<color=red><b>[SKFramework.Score.Error]</b></color> 加载配置文件失败");
                    }
                }
                return instance;
            }
        ...
        public string Create(int id)
        {
            ...
            if (info != null)
            {
                ...
                Log.Info("<color=cyan><b>[SKFramework.Score.Info]</b></color> 创建分数ID为[{0}]的分数项 [{1}]", id, info.description);
        ...
        public string[] CreateGroup(string groupDescription, ValueMode valueMode, params int[] idArray)
        {
            ...
            for (int i = 0; i < idArray.Length; i++)
            {
                ...
                if (info != null)
                {
                    ...
                    Log.Info("<color=cyan><b>[SKFramework.Score.Info]</b></color> 创建分数ID为[{0}]的分数项 [{1}]", idArray[i], info.description);
                }
                else
                {
