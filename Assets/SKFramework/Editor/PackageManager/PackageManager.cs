...
namespace SK.Framework
{
    ...
    public class PackageManager : EditorWindow
    {
        ...
        {
            var window = GetWindow<PackageManager>("Package Manager");
            window.minSize = new Vector2(600f, 300f);
            window.Show();
        }

        //服务器IP地址
        private const string ipAddress = "http://127.0.0.1:80";
        //manifest.json文件
        ...
        {
            //URL
            string url = string.Format("{0}/{1}", ipAddress, manifest);
            //发起网络请求
